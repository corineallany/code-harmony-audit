...
function Sollicitations() {
  ...
  const programName = (id: string | null | undefined) => activePrograms.find((p: any) => p.id === id)?.title ?? (programs.data ?? []).find((p: any) => p.id === id)?.title ?? "—";
  const creatorName = (s: any) => allMembers.find((m: any) => m.auth_user_id && m.auth_user_id === s.created_by)?.full_name ?? s.requester ?? "—";
  const selectedProgram = activePrograms.find((p: any) => p.id === form.program_id);
  const effectiveProgramDate = selectedProgram?.start_date ?? (form.event_date || "");
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const p = activePrograms.find((x: any) => x.id === form.program_id);
      const eventDate = form.event_date || p?.start_date || "";
      if (form.response_deadline < todayIso()) throw new Error("La date limite de réponse ne peut pas être dans le passé.");
      if (eventDate && form.response_deadline > eventDate) throw new Error("La date limite de réponse doit être au plus tard à la date du programme.");
  ...
  const extendDeadline = useMutation({
    ...
    onSuccess: async () => { toast.success("Date limite prolongée"); await qc.invalidateQueries({ queryKey: ["solicitations"] }); setDetail((d: any) => d ? { ...d, response_deadline: (solicitations.data ?? []).find((s: any) => s.id === d.id)?.response_deadline ?? d.response_deadline } : d); },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  return (
    <AppShell title="Sollicitations ponctuelles" subtitle="Renforts ponctuels et remplacements">
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}><DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">{detail ? <><DialogHeader><DialogTitle>{detail.event_name || "Sollicitation ponctuelle"}</DialogTitle></DialogHeader><div className="space-y-5"><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Info label="Nature" value={NLABEL[detail.nature] ?? "Sollicitation ponctuelle"} /><Info label="Date" value={formatDate(detail.event_date)} /><Info label="Programme" value={detail.program_id ? programName(detail.program_id) : "—"} /><Info label="Date limite" value={deadlineText(detail.response_deadline)} /><Info label="Statut" value={detail.status === "partial" ? "Réponses mixtes" : STATUS_LABEL[detail.status] ?? detail.status} /></div>
        ...
        {isStaff ? <div className="rounded-2xl border p-4"><h3 className="font-black text-icc-violet">Actions</h3><div className="mt-3 flex flex-wrap gap-2">{detail.status !== "cancelled" ? <Button size="sm" variant="outline" onClick={() => { setDetail(null); openEdit(detail); }}>Modifier</Button> : null}{detail.status !== "cancelled" && (!detail.event_date || detail.event_date >= todayIso()) ? <Button size="sm" variant="outline" onClick={() => { const max = detail.program_id ? (programs.data ?? []).find((p: any) => p.id === detail.program_id)?.start_date : detail.event_date; const date = window.prompt(`Nouvelle date limite${max ? ` (au plus tard ${formatDate(max)})` : ""} :`, detail.response_deadline ?? "") ?? ""; if (date) extendDeadline.mutate({ s: detail, date }); }}>Prolonger le délai</Button> : null}{detail.status !== "cancelled" ? <Button size="sm" variant="outline" onClick={() => { const note = window.prompt("Motif de l'annulation (facultatif) :") ?? ""; if (window.confirm("Confirmer l'annulation de cette sollicitation ?")) cancel.mutate({ s: detail, note }); }}>Annuler la demande</Button> : null}{isAdmin ? <Button size="sm" variant="ghost" onClick={() => { if (window.confirm("Archiver cette sollicitation ?")) archive.mutate(detail); }}>Archiver</Button> : null}</div><p className="mt-2 text-xs text-muted-foreground">Le délai peut être prolongé tant que la date du programme n'est pas passée, sans jamais la dépasser. Modifier permet aussi d'ajouter ou retirer des cibles si le besoin évolue.</p></div> : null}
