...
function Sollicitations() {
  ...
  const recipientTargets = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await (supabase as any).from("solicitation_recipient_targets").select("*");
      if (error) throw error;
  ...
  const docsForProgram = (id: string | null | undefined) =>
  ...
  const creatorName = (s: any) =>
    allMembers.find((m: any) => m.auth_user_id && m.auth_user_id === s.created_by)?.full_name ??
    s.requester ??
    "—";
  ...
  function resolveTargetMembers(t: TargetDraft) {
    ...
    return activeMembers
      .filter((m: any) =>
        t.ids.some((poleId) => links.some((l: any) => l.member_id === m.id && l.pole_id === poleId)),
      )
      .map((m: any) => m.id);
  }
  ...
  function openEdit(s: any) {
    const st = tgts(s.id);
    const draftTargets: TargetDraft[] = st.length
      ? st.reduce((acc: TargetDraft[], t) => {
    ...
    setForm({
      id: s.id,
      nature: s.nature === "replacement" || s.nature === "remplacement" ? "replacement" : "reinforcement",
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      const p =
        selectablePrograms.find((x: any) => x.id === form.program_id) ??
        allActivePrograms.find((x: any) => x.id === form.program_id);
      if (!p?.start_date) throw new Error("Le programme sélectionné doit avoir une date valide.");
      if (p.start_date < todayIso())
        throw new Error("Une sollicitation ponctuelle ne peut être créée que pour un programme à venir.");
      if (!form.response_deadline) throw new Error("La date limite de réponse est obligatoire.");
      if (form.response_deadline < todayIso())
        throw new Error("La date limite de réponse ne peut pas être dans le passé.");
      if (form.response_deadline > p.start_date)
        throw new Error("La date limite de réponse doit être au plus tard à la date du programme.");

      const usableTargets = form.targets.filter((t) => t.type === "all" || t.ids.length > 0);
      if (!usableTargets.length)
        throw new Error("Ajoute au moins une cible : personne, pôle ou Toute la COM.");
        ...
        for (const entityId of entityIds) {
          ...
          const name =
            t.type === "all"
              ...
              : t.type === "member"
                ? memberName(entityId)
                : activePoles.find((p: any) => p.id === entityId)?.name ?? "Pôle";
          targetRows.push({
            id: targetId,
            solicitation_id: id,
            target_type: t.type,
          ...
          targetMembers.forEach((memberId) =>
            memberTargetMap.set(memberId, [...(memberTargetMap.get(memberId) ?? []), targetId]),
          );
        }
      }
      ...
      if (added.length) {
        const rr = await (supabase as any)
          .from("solicitation_recipients")
          .insert(added.map((member_id) => ({ solicitation_id: id, member_id, response: "pending" })));
        if (rr.error) throw rr.error;
      }
      const mapRows = [...memberTargetMap.entries()].flatMap(([member_id, targetIds]) =>
        targetIds.map((target_id) => ({ solicitation_id: id, member_id, target_id })),
      );
      if (mapRows.length) {
      ...
      await logAction({
        ...
        entity: "solicitation",
        entityId: id,
        detail: `${p.title} · échéance ${form.response_deadline}`,
  ...
  async function applyOperationalEffect(s: any, r: Recipient) {
      ...
      if (assignmentIds.length) {
        ...
        for (const link of oldLinks ?? []) {
          const { data: exists } = await (supabase as any)
            ...
            .eq("assignment_id", link.assignment_id)
            .eq("member_id", memberId)
            .maybeSingle();
          if (!exists)
            await (supabase as any)
              .from("program_assignment_members")
              .insert({ assignment_id: link.assignment_id, member_id: memberId });
        }
        await (supabase as any)
          .from("program_assignment_members")
          .delete()
      ...
      await (supabase as any).from("program_member_responses").upsert(
        {
          ...
          member_id: memberId,
          status: "available",
          reason: `Remplacement accepté pour ${memberName(replacedId)}`,
        },
    ...
    await (supabase as any).from("program_member_responses").upsert(
      { program_id: s.program_id, member_id: memberId, status: "available", reason: "Renfort ponctuel accepté" },
      { onConflict: "program_id,member_id" },
    );
    const memberTargetIds = maps(s.id)
      .filter((x) => x.member_id === memberId)
      .map((x) => x.target_id);
    const poleTarget = tgts(s.id).find(
      (t) => memberTargetIds.includes(t.id) && t.target_type === "pole" && t.target_id,
    );
    ...
    if (!assignmentId) {
      assignmentId = uid("pa");
      await (supabase as any).from("program_assignments").insert({
        id: assignmentId,
        program_id: s.program_id,
        pole_id: poleId,
    ...
    if (!exists)
      await (supabase as any)
        .from("program_assignment_members")
        .insert({ assignment_id: assignmentId, member_id: memberId });
  }
  ...
  const answer = useMutation({
    mutationFn: async ({ s, r, response, note }: { s: any; r: Recipient; response: string; note?: string }) => {
      ...
      await logAction({
        action: "reponse_sollicitation",
        entity: "solicitation",
        entityId: s.id,
        detail: `${memberName(r.member_id)} : ${RLABEL[response]}`,
        actorName: member?.full_name,
      });
    },
    onSuccess: async () => {
  ...
  const extendDeadline = useMutation({
    mutationFn: async ({ s, date }: { s: any; date: string }) => {
      const programDate = s.program_id
        ? (programs.data ?? []).find((p: any) => p.id === s.program_id)?.start_date
        : s.event_date;
      ...
      if (programDate && programDate < todayIso())
        throw new Error("Le programme a déjà eu lieu : l'échéance ne peut plus être prolongée.");
      if (programDate && date > programDate)
        throw new Error("L'échéance ne peut pas dépasser la date du programme.");
      if (s.response_deadline && date <= s.response_deadline)
        throw new Error("La nouvelle date doit prolonger l'échéance actuelle.");
      const { error } = await (supabase as any)
        .from("solicitations")
        .update({ response_deadline: date, updated_at: new Date().toISOString() })
        .eq("id", s.id);
      if (error) throw error;
      await logAction({
        action: "echeance_sollicitation_prolongee",
        entity: "solicitation",
        entityId: s.id,
        detail: `Nouvelle échéance : ${date}`,
        actorName: member?.full_name,
      });
    },
    onSuccess: async () => {
      toast.success("Date limite prolongée");
      await qc.invalidateQueries({ queryKey: ["solicitations"] });
    },
    onError: (e: Error) => toast.error(e.message),
  });
  ...
  const cancel = useMutation({
    mutationFn: async ({ s, note }: { s: any; note: string }) => {
      ...
      if (error) throw error;
      await logAction({
        action: "sollicitation_annulee",
        entity: "solicitation",
        entityId: s.id,
        detail: note || s.event_name,
        actorName: member?.full_name,
      });
    },
    onSuccess: async () => {
  ...
  const archive = useMutation({
    ...
    onSuccess: async () => {
      toast.success("Sollicitation archivée");
      setDetail(null);
      await qc.invalidateQueries({ queryKey: ["solicitations"] });
    },
  });
  ...
  function needsForRecipient(sid: string, memberId: string) {
    const ids = maps(sid)
      ...
      .map((x) => x.target_id);
    return tgts(sid)
      .filter((t) => ids.includes(t.id) && t.need)
      .map((t) => `${t.target_name} : ${t.need}`);
  }
    ...
    <AppShell title="Sollicitations ponctuelles" subtitle="Renforts ponctuels et remplacements">
        ...
        {!rows.length ? (
          ...
          <div className="space-y-4">
            {rows.map((s: any) => {
              ...
              const files = docsForProgram(s.program_id);
              return (
                <Card key={s.id} className="cursor-pointer transition hover:border-icc-violet/40" onClick={() => setDetail(s)}>
                  <CardHeader>
                    <div className="flex flex-wrap justify-between gap-2">
                      <div>
                        ...
                        <p className="mt-1 text-sm text-muted-foreground">{NLABEL[s.nature ?? ""] ?? "Sollicitation ponctuelle"} · {formatDate(s.event_date)}</p>
                      </div>
                  ...
                  <CardContent className="space-y-2 text-sm">
                    ...
                    <p><b>Date limite :</b> <span className={s.response_deadline && s.response_deadline < todayIso() ? "font-bold text-red-600" : ""}>{deadlineText(s.response_deadline)}</span></p>
                    <p><b>Cibles :</b> {tgts(s.id).length ? tgts(s.id).map((t) => t.target_name).join(" · ") : s.target_name || "—"}</p>
                    {files.length ? <p><b>Fichiers programme :</b> {files.length}</p> : null}
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
          ...
          <div className="space-y-5">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label="Programme lié *">
                <Select value={form.program_id || undefined} onValueChange={(pid) => { const p = selectablePrograms.find((x: any) => x.id === pid); setForm({ ...form, program_id: pid, event_name: p?.title ?? "", event_date: p?.start_date ?? "", response_deadline: p?.response_deadline ?? "" }); }}>
                  <SelectTrigger><SelectValue placeholder="Sélectionner un programme" /></SelectTrigger>
                  <SelectContent>{selectablePrograms.map((p: any) => <SelectItem key={p.id} value={p.id}>{p.title} · {formatDate(p.start_date)}</SelectItem>)}</SelectContent>
                </Select>
                {selectablePrograms.length === 0 ? <span className="block text-[11px] text-red-600">Aucun programme à venir disponible.</span> : null}
              </Field>
              <Info label="Date du programme" value={selectedProgram?.start_date ? formatDate(selectedProgram.start_date) : "Sélectionne d'abord un programme"} />
            ...
            <section className="space-y-3">
              <div className="flex items-center justify-between gap-2">
                <div><h3 className="font-black text-icc-violet">Cibles et besoins</h3><p className="text-xs text-muted-foreground">Tu peux cumuler personnes, pôles et Toute la COM. Chaque bloc peut avoir son besoin propre.</p></div>
                <Button type="button" size="sm" variant="outline" onClick={() => setForm({ ...form, targets: [...form.targets, newTarget()] })}>+ Ajouter une cible</Button>
              </div>
          ...
          <DialogFooter><Button variant="ghost" onClick={() => setFormOpen(false)}>Annuler</Button><Button disabled={save.isPending || !form.program_id} onClick={() => save.mutate()}>{form.id ? "Enregistrer les modifications" : "Créer et envoyer"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
          {detail ? <>
            <DialogHeader><DialogTitle>{programName(detail.program_id)}</DialogTitle></DialogHeader>
            <div className="space-y-5">
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Info label="Nature" value={NLABEL[detail.nature] ?? "Sollicitation ponctuelle"} /><Info label="Date" value={formatDate(detail.event_date)} /><Info label="Programme" value={programName(detail.program_id)} /><Info label="Date limite" value={deadlineText(detail.response_deadline)} /><Info label="Statut" value={detail.status === "partial" ? "Réponses mixtes" : STATUS_LABEL[detail.status] ?? detail.status} /></div>

              <div className="rounded-2xl border p-4">
                <h3 className="font-black text-icc-violet">Origine de la demande</h3>
                <div className="mt-3 grid gap-3 sm:grid-cols-2"><Info label={detail.nature === "replacement" ? "Personne remplacée" : "Personne à l'origine du besoin"} value={memberName(detail.requester_member_id) || detail.requester || "—"} /><Info label="Demande saisie par" value={creatorName(detail)} /></div>
              ...
              <div className="rounded-2xl border p-4">
                ...
                <div className="mt-3 grid gap-3 md:grid-cols-2">{recs(detail.id).map((r) => { const needs = needsForRecipient(detail.id, r.member_id); return <div key={r.id} className="rounded-xl border p-3 text-sm"><div className="flex flex-wrap items-start justify-between gap-2"><b>{memberName(r.member_id)}</b><Badge variant={r.response === "pending" ? "outline" : "secondary"}>{RLABEL[r.response] ?? r.response}</Badge></div>{needs.length ? <div className="mt-2 text-xs text-muted-foreground">{needs.map((n) => <p key={n}>• {n}</p>)}</div> : null}{r.reserve ? <p className="mt-2"><b>Réserve :</b> {r.reserve}</p> : null}{r.refusal_reason ? <p className="mt-2"><b>Motif :</b> {r.refusal_reason}</p> : null}{member?.id === r.member_id && r.response === "pending" && detail.status !== "cancelled" ? <div className="mt-3 flex flex-wrap gap-1"><Button size="sm" onClick={() => respond(detail, r, "accepted")}>Accepter</Button><Button size="sm" variant="outline" onClick={() => respond(detail, r, "partial")}>Accepter partiellement</Button><Button size="sm" variant="outline" onClick={() => respond(detail, r, "refused")}>Refuser</Button></div> : null}</div>; })}</div>
              </div>
    ...
    <div className="rounded-2xl border p-4">
      ...
      {files.length ? (
        <div className="mt-3 grid gap-2 sm:grid-cols-2">
          {files.map((f) => (
            <a key={f.id} href={f.url} target="_blank" rel="noreferrer" className="rounded-xl bg-muted/50 p-3 text-sm font-semibold text-icc-violet hover:underline">📎 {f.title}</a>
          ))}
        </div>
      ) : <p className="mt-2 text-sm text-muted-foreground">Aucun fichier joint à ce programme.</p>}
      <p className="mt-2 text-xs text-muted-foreground">Cette liste provient directement de la fiche programme : aucune copie n'est créée dans la sollicitation.</p>
    </div>
  );
}
...
function Filter({ value, set, all, items }: { value: string; set: (v: string) => void; all: string; items: string[][] }) {
  return <Select value={value} onValueChange={set}><SelectTrigger><SelectValue placeholder={all} /></SelectTrigger><SelectContent>{value === "all" || !items.some((i) => i[0] === "all") ? <SelectItem value="all">{all}</SelectItem> : null}{items.map(([v, l]) => <SelectItem key={v} value={v}>{l}</SelectItem>)}</SelectContent></Select>;
}
function Checks({ items, values, set }: { items: string[][]; values: string[]; set: (v: string[]) => void }) {
  return <div className="mt-3 max-h-52 space-y-1 overflow-y-auto rounded-xl border p-2">{items.map(([id, label]) => <label key={id} className="flex items-center gap-2 rounded-lg p-2 hover:bg-muted"><Checkbox checked={values.includes(id)} onCheckedChange={(c) => set(c ? [...values, id] : values.filter((x) => x !== id))} />{label}</label>)}</div>;
}
