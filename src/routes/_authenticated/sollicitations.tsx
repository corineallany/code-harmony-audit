...
type FormState = { id: string | null; nature: "reinforcement" | "replacement"; requester_member_id: string; program_id: string; event_name: string; event_date: string; message: string; targets: TargetDraft[] };
...
const blankForm = (requester = ""): FormState => ({ id: null, nature: "reinforcement", requester_member_id: requester, program_id: "", event_name: "", event_date: "", message: "", targets: [newTarget()] });
...
function Sollicitations() {
  ...
  const activePrograms = (programs.data ?? []).filter((p) => !p.archived && !p.deleted);

  const rows = useMemo(() => {
    let data = (solicitations.data ?? []).filter((s) => !s.archived);
    ...
    if (program !== "all") data = data.filter((s) => s.program_id === program);
    return [...data].sort((a, b) => sort === "old" ? String(a.event_date ?? a.created_at).localeCompare(String(b.event_date ?? b.created_at)) : String(b.event_date ?? b.created_at).localeCompare(String(a.event_date ?? a.created_at)));
  ...
  const programName = (id: string | null | undefined) => activePrograms.find((p) => p.id === id)?.title ?? (programs.data ?? []).find((p) => p.id === id)?.title ?? "—";
  ...
  function resolveTargetMembers(t: TargetDraft) {
    ...
    return activeMembers.filter((m: any) => t.ids.some((poleId) => links.some((l) => l.member_id === m.id && l.pole_id === poleId))).map((m: any) => m.id);
  }
  ...
  function targetDisplay(t: TargetDraft) {
    ...
    return t.ids.map((id) => activePoles.find((p) => p.id === id)?.name ?? "Pôle").join(", ");
  }
  ...
  function openEdit(s: any) {
    ...
    setForm({ id: s.id, nature: (s.nature === "replacement" || s.nature === "remplacement") ? "replacement" : "reinforcement", requester_member_id: s.requester_member_id ?? member?.id ?? "", program_id: s.program_id ?? "", event_name: s.event_name ?? "", event_date: s.event_date ?? "", message: s.message ?? "", targets: draftTargets });
    setFormOpen(true);
  }
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (!form.event_name.trim() && !form.program_id) throw new Error("Renseigne un programme ou un intitulé.");
      ...
      const p = activePrograms.find((x) => x.id === form.program_id);
      ...
      const payload: any = {
        ...
        event_name: form.event_name.trim() || p?.title || "Sollicitation ponctuelle",
        event_date: form.event_date || p?.start_date || null,
      ...
      for (const t of usableTargets) {
        ...
        for (const entityId of entityIds) {
          ...
          const name = t.type === "all" ? "Toute la COM" : t.type === "member" ? memberName(entityId) : activePoles.find((p) => p.id === entityId)?.name ?? "Pôle";
          ...
          const targetMembers = t.type === "all" ? resolveTargetMembers(t) : t.type === "member" ? [entityId as string] : activeMembers.filter((m: any) => links.some((l) => l.member_id === m.id && l.pole_id === entityId)).map((m: any) => m.id);
      ...
      if (added.length) {
        const rr = await (supabase as any).from("solicitation_recipients").insert(added.map((member_id) => ({ solicitation_id: id, member_id, response: "pending" })));
      ...
      await logAction({ action: form.id ? "sollicitation_modifiee" : "sollicitation_creee", entity: "solicitation", entityId: id, detail: payload.event_name, actorName: member?.full_name });
    },
    onSuccess: async () => {
  ...
  async function applyOperationalEffect(s: any, r: Recipient) {
    ...
    const fallbackPole = links.find((l) => l.member_id === memberId)?.pole_id;
    ...
    if (!assignmentId) {
      assignmentId = uid("pa");
      const need = poleTarget?.need ?? "Renfort ponctuel";
      await (supabase as any).from("program_assignments").insert({ id: assignmentId, program_id: s.program_id, pole_id: poleId, tasks: need });
    }
  ...
  const answer = useMutation({
    mutationFn: async ({ s, r, response, note }: { s: any; r: Recipient; response: string; note?: string }) => {
      ...
      const { error } = await (supabase as any).from("solicitation_recipients").update(patch).eq("id", r.id);
    ...
    onSuccess: async () => {
      ...
      await Promise.all([qc.invalidateQueries({ queryKey: ["solicitation-recipients"] }), qc.invalidateQueries({ queryKey: ["solicitations"] }), qc.invalidateQueries({ queryKey: ["programs"] })]);
    },
    onError: (e: Error) => toast.error(e.message),
  });

  const cancel = useMutation({
    mutationFn: async ({ s, note }: { s: any; note: string }) => {
      const { error } = await (supabase as any).from("solicitations").update({ status: "cancelled", cancellation_note: note || null, cancelled_at: new Date().toISOString(), cancelled_by: member?.full_name ?? null }).eq("id", s.id);
      if (error) throw error;
      await logAction({ action: "sollicitation_annulee", entity: "solicitation", entityId: s.id, detail: note || s.event_name, actorName: member?.full_name });
    },
    onSuccess: async () => { toast.success("Sollicitation annulée"); setDetail(null); await qc.invalidateQueries({ queryKey: ["solicitations"] }); },
    onError: (e: Error) => toast.error(e.message),
  });

  const archive = useMutation({
    mutationFn: async (s: any) => { const { error } = await (supabase as any).from("solicitations").update({ archived: true, archived_at: new Date().toISOString() }).eq("id", s.id); if (error) throw error; },
    onSuccess: async () => { toast.success("Sollicitation archivée"); setDetail(null); await qc.invalidateQueries({ queryKey: ["solicitations"] }); },
  });
  ...
  function needsForRecipient(sid: string, memberId: string) {
    const ids = maps(sid).filter((x) => x.member_id === memberId).map((x) => x.target_id);
    return tgts(sid).filter((t) => ids.includes(t.id) && t.need).map((t) => `${t.target_name} : ${t.need}`);
  }
    ...
    <AppShell title="Sollicitations ponctuelles" subtitle="Renforts ponctuels et remplacements">
      <div className="space-y-4">
        ...
        <div className="grid gap-2 md:grid-cols-3 xl:grid-cols-6">
          ...
          <Filter value={program} set={setProgram} all="Tous les programmes" items={activePrograms.map((p) => [p.id, p.title])} />
          <Filter value={sort} set={setSort} all="Tri" items={[["recent", "Plus récentes"], ["old", "Plus anciennes"]]} />
        </div>

        {!rows.length ? <EmptyState title="Aucune sollicitation" /> : (
          <div className="space-y-4">
            {rows.map((s: any) => {
              const rs = recs(s.id);
              const agg = s.status === "cancelled" ? "cancelled" : aggregate(rs) ?? s.status;
              return (
                <Card key={s.id} className="cursor-pointer transition hover:border-icc-violet/40" onClick={() => setDetail(s)}>
                  <CardHeader>
                    <div className="flex flex-wrap justify-between gap-2">
                      <div>
                        <CardTitle className="text-lg">{s.event_name || programName(s.program_id)}</CardTitle>
                        <p className="mt-1 text-sm text-muted-foreground">{NLABEL[s.nature ?? ""] ?? "Sollicitation ponctuelle"} · {formatDate(s.event_date)}{s.program_id ? ` · ${programName(s.program_id)}` : ""}</p>
                      </div>
                      <Badge variant={agg === "pending" ? "outline" : "secondary"}>{agg === "partial" ? "Réponses mixtes" : STATUS_LABEL[agg] ?? RLABEL[agg] ?? agg}</Badge>
                    </div>
                  </CardHeader>
                  <CardContent className="space-y-2 text-sm">
                    <p><b>{s.nature === "replacement" ? "Personne remplacée" : "Demandeur"} :</b> {memberName(s.requester_member_id) || s.requester || "—"}</p>
                    <p><b>Cibles :</b> {tgts(s.id).length ? tgts(s.id).map((t) => t.target_name).join(" · ") : s.target_name || "—"}</p>
                    <p className="text-muted-foreground">{rs.length} personne(s) sollicitée(s) · cliquer pour ouvrir la fiche complète</p>
                  </CardContent>
                </Card>
              );
            })}
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
          <DialogHeader><DialogTitle>{form.id ? "Modifier la sollicitation" : "Nouvelle sollicitation ponctuelle"}</DialogTitle></DialogHeader>
          <div className="space-y-5">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Field label={form.nature === "replacement" ? "Personne à remplacer / à l'origine du besoin" : "Personne à l'origine du besoin"}><Filter value={form.requester_member_id || "none"} set={(v) => setForm({ ...form, requester_member_id: v === "none" ? "" : v })} all="Choisir" items={activeMembers.map((m: any) => [m.id, m.full_name])} /></Field>
              <Field label="Programme lié"><Filter value={form.program_id || "none"} set={(v) => setForm({ ...form, program_id: v === "none" ? "" : v })} all="Programme" items={[["none", "Aucun programme"], ...activePrograms.map((p) => [p.id, p.title])]} /></Field>
              <Field label="Date"><Input type="date" value={form.event_date} onChange={(e) => setForm({ ...form, event_date: e.target.value })} /></Field>
              <Field label="Intitulé"><Input value={form.event_name} onChange={(e) => setForm({ ...form, event_name: e.target.value })} /></Field>
            </div>
            <Field label="Contexte général / précision"><Textarea value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} /></Field>

            <section className="space-y-3">
              <div className="flex items-center justify-between gap-2"><div><h3 className="font-black text-icc-violet">Cibles et besoins</h3><p className="text-xs text-muted-foreground">Tu peux cumuler personnes, pôles et Toute la COM. Chaque bloc peut avoir son besoin propre.</p></div><Button type="button" size="sm" variant="outline" onClick={() => setForm({ ...form, targets: [...form.targets, newTarget()] })}>+ Ajouter une cible</Button></div>
              {form.targets.map((t, index) => (
                <div key={t.key} className="rounded-2xl border p-4">
                  <div className="flex items-start justify-between gap-3"><b>Cible {index + 1}</b>{form.targets.length > 1 ? <Button type="button" size="sm" variant="ghost" onClick={() => setForm({ ...form, targets: form.targets.filter((x) => x.key !== t.key) })}>Retirer</Button> : null}</div>
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    <Field label="Type de cible"><Filter value={t.type} set={(v) => setForm({ ...form, targets: form.targets.map((x) => x.key === t.key ? { ...x, type: v as TargetDraft["type"], ids: [] } : x) })} all="Type" items={[["member", "Une ou plusieurs personnes"], ["pole", "Un ou plusieurs pôles"], ["all", "Toute la COM"]]} /></Field>
                    <Field label="Besoin spécifique"><Input placeholder="Ex. prise de photos, renfort montage…" value={t.need} onChange={(e) => setForm({ ...form, targets: form.targets.map((x) => x.key === t.key ? { ...x, need: e.target.value } : x) })} /></Field>
                  </div>
                  {t.type === "member" ? <Checks items={activeMembers.map((m: any) => [m.id, m.full_name])} values={t.ids} set={(ids) => setForm({ ...form, targets: form.targets.map((x) => x.key === t.key ? { ...x, ids } : x) })} /> : null}
                  {t.type === "pole" ? <Checks items={activePoles.map((p) => [p.id, p.name])} values={t.ids} set={(ids) => setForm({ ...form, targets: form.targets.map((x) => x.key === t.key ? { ...x, ids } : x) })} /> : null}
                  {t.type === "all" ? <p className="mt-3 rounded-xl bg-muted/50 p-3 text-sm">Tous les membres actifs de la COM seront enregistrés individuellement comme destinataires.</p> : null}
                </div>
              ))}
            </section>
          </div>
          <DialogFooter><Button variant="ghost" onClick={() => setFormOpen(false)}>Annuler</Button><Button disabled={save.isPending} onClick={() => save.mutate()}>{form.id ? "Enregistrer les modifications" : "Créer et envoyer"}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[92vh] max-w-5xl overflow-y-auto">
          {detail ? (
            <>
              <DialogHeader><DialogTitle>{detail.event_name || "Sollicitation ponctuelle"}</DialogTitle></DialogHeader>
              <div className="space-y-5">
                <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                  <Info label="Nature" value={NLABEL[detail.nature] ?? "Sollicitation ponctuelle"} />
                  <Info label="Date" value={formatDate(detail.event_date)} />
                  <Info label="Programme" value={detail.program_id ? programName(detail.program_id) : "—"} />
                  <Info label="Statut" value={detail.status === "partial" ? "Réponses mixtes" : STATUS_LABEL[detail.status] ?? detail.status} />
                </div>

                <div className="rounded-2xl border p-4">
                  <h3 className="font-black text-icc-violet">Origine de la demande</h3>
                  <div className="mt-3 grid gap-3 sm:grid-cols-2">
                    <Info label={detail.nature === "replacement" ? "Personne remplacée" : "Personne à l'origine du besoin"} value={memberName(detail.requester_member_id) || detail.requester || "—"} />
                    <Info label="Demande saisie par" value={creatorName(detail)} />
                  </div>
                  {detail.message ? <div className="mt-3 rounded-xl bg-muted/40 p-3 text-sm"><b>Contexte général</b><p className="mt-1">{detail.message}</p></div> : null}
                </div>

                <div className="rounded-2xl border p-4">
                  <h3 className="font-black text-icc-violet">Cibles et besoins</h3>
                  <div className="mt-3 space-y-2">{tgts(detail.id).length ? tgts(detail.id).map((t) => <div key={t.id} className="rounded-xl bg-muted/40 p-3"><div className="flex flex-wrap items-center justify-between gap-2"><b>{t.target_name}</b><Badge variant="outline">{t.target_type === "member" ? "Personne" : t.target_type === "pole" ? "Pôle" : "Toute la COM"}</Badge></div><p className="mt-1 text-sm">{t.need || "Besoin non précisé"}</p></div>) : <p className="text-sm text-muted-foreground">{detail.target_name || "—"}</p>}</div>
                </div>

                <div className="rounded-2xl border p-4">
                  <div className="flex flex-wrap items-center justify-between gap-2"><h3 className="font-black text-icc-violet">Réponses individuelles</h3><span className="text-sm text-muted-foreground">{recs(detail.id).length} personne(s)</span></div>
                  <div className="mt-3 grid gap-3 md:grid-cols-2">
                    {recs(detail.id).map((r) => {
                      const needs = needsForRecipient(detail.id, r.member_id);
                      return <div key={r.id} className="rounded-xl border p-3 text-sm"><div className="flex flex-wrap items-start justify-between gap-2"><b>{memberName(r.member_id)}</b><Badge variant={r.response === "pending" ? "outline" : "secondary"}>{RLABEL[r.response] ?? r.response}</Badge></div>{needs.length ? <div className="mt-2 text-xs text-muted-foreground">{needs.map((n) => <p key={n}>• {n}</p>)}</div> : null}{r.reserve ? <p className="mt-2"><b>Réserve :</b> {r.reserve}</p> : null}{r.refusal_reason ? <p className="mt-2"><b>Motif :</b> {r.refusal_reason}</p> : null}{member?.id === r.member_id && r.response === "pending" && detail.status !== "cancelled" ? <div className="mt-3 flex flex-wrap gap-1"><Button size="sm" onClick={() => respond(detail, r, "accepted")}>Accepter</Button><Button size="sm" variant="outline" onClick={() => respond(detail, r, "partial")}>Accepter partiellement</Button><Button size="sm" variant="outline" onClick={() => respond(detail, r, "refused")}>Refuser</Button></div> : null}</div>;
                    })}
                  </div>
                </div>
                ...
                {isStaff ? <div className="rounded-2xl border p-4"><h3 className="font-black text-icc-violet">Actions</h3><div className="mt-3 flex flex-wrap gap-2">{detail.status !== "cancelled" ? <Button size="sm" variant="outline" onClick={() => { setDetail(null); openEdit(detail); }}>Modifier</Button> : null}{detail.status !== "cancelled" ? <Button size="sm" variant="outline" onClick={() => { const note = window.prompt("Motif de l'annulation (facultatif) :") ?? ""; if (window.confirm("Confirmer l'annulation de cette sollicitation ?")) cancel.mutate({ s: detail, note }); }}>Annuler la demande</Button> : null}{isAdmin ? <Button size="sm" variant="ghost" onClick={() => { if (window.confirm("Archiver cette sollicitation ?")) archive.mutate(detail); }}>Archiver</Button> : null}</div><p className="mt-2 text-xs text-muted-foreground">Modifier permet d'ajouter ou retirer des cibles si le besoin évolue. Les réponses déjà reçues sont conservées pour les personnes qui restent sollicitées.</p></div> : null}
              </div>
            </>
          ) : null}
        </DialogContent>
