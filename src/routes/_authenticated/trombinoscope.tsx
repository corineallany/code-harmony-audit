...
import { ROLE_LABEL } from "@/hooks/useAuth";
...
function Trombinoscope() {
  ...
  const upcoming = useMemo(() => rows.filter((m) => daysUntilBirthday(m) <= 7), [rows]);
  ...
  return (
    <AppShell title="Trombinoscope" subtitle={`${rows.length} équipier(s)`}>
      ...
      <div className="mb-5 grid gap-2 md:grid-cols-2 xl:grid-cols-5">
        <Input placeholder="Rechercher un équipier…" value={search} onChange={(e) => setSearch(e.target.value)} />
      ...
      ) : rows.length === 0 ? <EmptyState title="Aucun équipier trouvé" /> : (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {rows.map((m) => {
            const state = memberState(m);
            const arrival = m.arrival_year ? `${m.arrival_month ? `${MONTHS[m.arrival_month - 1]} ` : ""}${m.arrival_year}` : null;
            return (
              <Card key={m.id} className={state === "archived" ? "opacity-50" : state === "inactive" ? "opacity-70" : ""}>
                <CardContent className="p-4">
                  <div className="flex gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-2 flex flex-wrap gap-1">
                        <Badge variant="outline">🏅 {ROLE_LABEL[m.base_role] ?? m.base_role}</Badge>
                        {m.is_icc ? <Badge variant="outline">✨ ICC</Badge> : null}
                        {m.is_ejp ? <Badge variant="outline">⭐ EJP</Badge> : null}
                        {m.poles.some((p: any) => p.referent) ? <Badge variant="outline">🏅 Référent</Badge> : null}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="mt-3 flex flex-wrap gap-1">
                    {m.poles.length === 0 ? <span className="text-xs text-muted-foreground">Sans pôle</span> : m.poles.map((p: any) => (
                      <Badge key={p.id} variant={p.referent ? "default" : "secondary"} className="text-[0.7rem]">{p.name}{p.referent ? " · référent" : ""}</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
