...
export type Pole = Tables["poles"]["Row"]; export type Member = Tables["members"]["Row"]; export type Program = Tables["programs"]["Row"]; export type Solicitation = Tables["solicitations"]["Row"]; export type AuditEntry = Tables["audit_log"]["Row"]; export type AppSettings = Tables["app_settings"]["Row"]; export type Notification = Tables["notifications"]["Row"]; export type ResponseStatus = Database["public"]["Enums"]["response_status"];
...
export const membersQuery=queryOptions({queryKey:["members"],queryFn:async()=>{const[members,links]=await Promise.all([supabase.from("members").select("*").order("full_name"),supabase.from("member_poles").select("member_id, pole_id, is_referent")]);return{members:unwrap(members),links:unwrap(links)}}});
export type ProgramWithDetails=Program&{assignments:Array<{id:string;pole_id:string;tasks:string|null;required_count:number;memberIds:string[]}>;responses:Array<{member_id:string;status:ResponseStatus;reason:string|null}>};
export const programsQuery=queryOptions({queryKey:["programs"],queryFn:async():Promise<ProgramWithDetails[]>=>{const[programs,assignments,assignMembers,responses]=await Promise.all([supabase.from("programs").select("*").eq("deleted",false).order("start_date"),(supabase as any).from("program_assignments").select("id, program_id, pole_id, tasks, required_count"),supabase.from("program_assignment_members").select("assignment_id, member_id"),supabase.from("program_member_responses").select("program_id, member_id, status, reason")]);const rows=unwrap(programs),asg=unwrap(assignments)as any[],asgMembers=unwrap(assignMembers),resp=unwrap(responses);return rows.map(p=>({...p,assignments:asg.filter(a=>a.program_id===p.id).map(a=>({id:a.id,pole_id:a.pole_id,tasks:a.tasks,required_count:Number(a.required_count??0),memberIds:asgMembers.filter(m=>m.assignment_id===a.id).map(m=>m.member_id)})),responses:resp.filter(r=>r.program_id===p.id).map(r=>({member_id:r.member_id,status:r.status,reason:r.reason}))}))}});
...
export const TASK_STATUS_LABEL:Record<string,string>={todo:"À faire",doing:"En cours",done:"Terminée"}; export const TASK_PRIORITY_LABEL:Record<string,string>={basse:"Basse",normale:"Normale",haute:"Haute"};
export type ProgramModel=Tables["program_models"]["Row"]; export type ProgramDebrief=Tables["program_debriefs"]["Row"]; export type EvaluationRow=Tables["evaluations"]["Row"];
...
export const debriefsQuery=queryOptions({queryKey:["program-debriefs"],queryFn:async()=>unwrap(await supabase.from("program_debriefs").select("*").order("created_at",{ascending:false}))});
export const evaluationsQuery=queryOptions({queryKey:["evaluations"],queryFn:async()=>unwrap(await supabase.from("evaluations").select("*").order("created_at",{ascending:false}))});
export const AVAILABILITY_STATUS_LABEL:Record<string,string>={pending:"En attente de validation",validated:"Validée",refused:"Refusée"};
export type Attendance=Tables["program_attendance"]["Row"]; export type ProgramDocument=Tables["program_documents"]["Row"]; export type InternalNote=Tables["internal_notes"]["Row"]; export type NotificationPreference=Tables["notification_preferences"]["Row"];
export const PRESENCE_LABEL:Record<string,string>={present:"Présent",absent:"Absent",retard:"Retard",partiel:"Présence partielle",remplace:"Remplacé",renfort:"Renfort"}; export const COMPLETION_LABEL:Record<string,string>={total:"Réalisé totalement",partiel:"Réalisé partiellement",non:"Non réalisé"};
export const INCIDENT_TYPES=["Technique","Organisation","Matériel","Retard-absence","Communication","Autre"]as const;
export const NOTIFICATION_EVENTS:Array<{key:string;label:string}>=[{key:"programme_affectation",label:"Je suis affecté à un programme"},{key:"programme_rappel",label:"Rappel avant un service"},{key:"programme_modification",label:"Un programme qui me concerne change"},{key:"reponse_recue",label:"Une réponse est reçue sur mon programme"},{key:"sollicitation",label:"Nouvelle sollicitation ponctuelle"},{key:"indisponibilite",label:"Indisponibilité à valider / décidée"},{key:"evaluation",label:"Évaluation à rédiger, à valider ou validée"},{key:"post_service",label:"Post-service à compléter"},{key:"tache",label:"Tâche qui m'est assignée"}];
export const attendanceQuery=queryOptions({queryKey:["program-attendance"],queryFn:async()=>unwrap(await supabase.from("program_attendance").select("*"))});
export const programDocumentsQuery=queryOptions({queryKey:["program-documents"],queryFn:async()=>unwrap(await supabase.from("program_documents").select("*").order("created_at"))});
