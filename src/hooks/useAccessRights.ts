...
const rank:Record<AccessScope,number>={interdit:0,moi:1,mon_pole:2,tous:3};
export function useAccessRights(){
 ...
 const scope=(moduleKey:string,actionKey:string):AccessScope=>{const exact=(exceptions.data??[]).find((x:any)=>x.module_key===moduleKey&&x.action_key===actionKey);if(exact)return exact.enabled===false?"interdit":(exact.scope??"interdit");const moduleExceptions=(exceptions.data??[]).filter((x:any)=>x.module_key===moduleKey&&x.enabled!==false).map((x:any)=>(x.scope??"interdit") as AccessScope);const inherited=moduleExceptions.sort((a:AccessScope,b:AccessScope)=>rank[b]-rank[a])[0];if(inherited==="tous")return"tous";const row=(perms.data??[]).find((x:any)=>x.role_key===roleKey&&x.module_key===moduleKey&&x.action_key===actionKey);return row?.enabled===false?"interdit":(row?.scope??"interdit")};
