      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3">
        <Link to="/tableau-de-bord" className="flex items-center gap-3 text-left">
          ...
          <span className="leading-none">
            <span className="block text-lg font-black tracking-[.08em] text-white md:text-xl">
              {brand}
            </span>
            <span className="mt-1 block text-[8px] font-semibold tracking-wide text-white/75 md:text-[9px]">
              {subtitle}
            </span>
          </span>
        </Link>
        ...
        <div className="flex items-center gap-2">
          ...
          {isStaff ? (
            <Link
              ...
              className="rounded-lg p-2 text-base leading-none transition-colors hover:bg-icc-violet-hover"
            >
              ⚙️
            </Link>
          ) : null}
          <div className="relative">
            <button
              ...
              onClick={() => setOpen((o) => !o)}
              title="Mon compte"
              className="flex items-center gap-2 rounded-lg border border-white/30 bg-icc-violet-hover py-1.5 pl-1.5 pr-3 transition-colors hover:bg-white/10"
            >
            ...
            {open ? (
              <div className="absolute right-0 z-[80] mt-2 w-64 overflow-hidden rounded-xl border border-border bg-white p-4 text-slate-800 shadow-2xl">
                ...
                <button
                  type="button"
                  onClick={signOut}
                  className="mt-3 w-full rounded-lg bg-icc-violet px-3 py-2 text-xs font-bold text-white"
                >
                  Se déconnecter
                </button>
              </div>
