...
export function HomeMenuGrid(){const{isStaff,isAdmin}=useCurrentRole();const settings=useQuery(settingsQuery);const config=normalizeMenuModules((settings.data as any)?.menus);const byKey=new Map(config.map(x=>[x.key,x]));const items=ITEMS.map(item=>{const c=byKey.get(item.key);return {...item,title:c?.label||item.title,_enabled:c?.enabled!==false,_visible:c?.menuVisible!==false,_order:c?.order??999}}).filter(i=>i._enabled&&i._visible).sort((a,b)=>a._order-b._order);return <><div className="mt-6 rounded-2xl border border-violet-200 bg-violet-50/50 p-4 text-sm"><b className="text-icc-violet">Tous les modules sont visibles par tous.</b><p className="mt-1 text-muted-foreground">La visibilité permet à chacun de comprendre tout ce que l’application propose. Les actions restent limitées selon le rôle et les droits ; les données sensibles ne sont jamais ouvertes simplement parce qu’un module est visible.</p></div><div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{items.map(item=>{const restricted=(item.adminOnly&&!isAdmin)||(item.staffOnly&&!isStaff);const target=item.key==="sollicitations"&&!isStaff?"/mes-demandes":restricted?`/decouvrir-module?module=${item.key}`:item.to;return <Link key={item.key} to={target as any} className={cn("icc-menu-card block",item.tone==="violet"&&"bg-icc-violet text-white hover:bg-icc-violet-hover",item.tone==="dark"&&"border-slate-800 bg-slate-800 text-white hover:bg-slate-900")}><div className="flex items-start justify-between gap-2"><span className={cn("text-2xl",item.tone?"text-icc-yellow":"text-icc-violet")}>{item.icon}</span>{restricted?<span className={cn("rounded-full px-2 py-1 text-[10px] font-bold",item.tone?"bg-white/15 text-white":"bg-muted text-muted-foreground")}>Consultation</span>:null}</div><h3 className="mt-2.5 font-black">{item.title}</h3><p className={cn("mt-1 text-xs",item.tone?"text-white/70":"text-muted-foreground")}>{item.key==="sollicitations"&&!isStaff?"Demander un remplacement ou signaler un besoin de renfort.":item.desc}</p></Link>})}</div></>}
